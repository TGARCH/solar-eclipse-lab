...
function App(){
 ...
 const saveViewPng=()=>{const state=canvasRef.current;if(!state)return;state.gl.render(state.scene,state.camera);const a=document.createElement('a');a.href=state.gl.domElement.toDataURL('image/png');a.download=`pzt-cienie-${solar.date}-${String(solar.hour).padStart(2,'0')}.png`;a.click()}
