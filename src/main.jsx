...
const SYSTEM_PRESETS=[
 ...
 {id:'gas-floor',name:'Kocioł gazowy + podłogówka',tag:'WARIANT',desc:'Kocioł kondensacyjny pracuje efektywniej z niskotemperaturowym ogrzewaniem podłogowym.',values:{heatingSystem:'Kocioł gazowy kondensacyjny + ogrzewanie podłogowe',energyCarrier:'Gaz ziemny',heatingGeneration:'0.96',heatingDistribution:'0.96',heatingControl:'0.93',ventilationType:'Wentylacja mechaniczna z odzyskiem',heatRecovery:'80',dhwSystem:'Kocioł gazowy',dhwEfficiency:'0.84',primaryFactor:'1.10'}},
...
function EnergyWorkspace({values,onChange}){
 const [step,setStep]=useState('building'),[advanced,setAdvanced]=useState(false),[calcState,setCalcState]=useState('idle'),[zones,setZones]=useState([{id:1,name:'Strefa ogrzewana',type:'heated',area:values.heatedArea||'',temperature:values.indoorTemp||'21',source:'IFC'}])
 ...
 const updateZone=(id,key,value)=>{setZones(list=>list.map(z=>z.id===id?{...z,[key]:value}:z));setCalcState('idle')}
 const addZone=()=>setZones(list=>[...list,{id:Date.now(),name:'Nowa strefa',type:'unheated',area:'',temperature:'8',source:'Ręczne'}])
 const removeZone=id=>setZones(list=>list.length>1?list.filter(z=>z.id!==id):list)
 const calculate=()=>{
  const zonedArea=zones.filter(z=>z.type==='heated').reduce((sum,z)=>sum+(Number(String(z.area).replace(',','.'))||0),0)
  const area=Math.max(zonedArea||n('heatedArea'),1),volume=Math.max(n('heatedVolume'),1)
 ...
 const exportEnergy=()=>{const payload={format:'Projektowana charakterystyka energetyczna',version:3,status:calcState,generatedAt:new Date().toISOString(),zones,values};const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));a.download='dane-charakterystyki-energetycznej.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
 const reportRows=[
 ...
 return <div className="energy-workspace">
  ...
  {step==='building'?<div className="building-zones">
   <div className="zones-head"><div><b>Strefy cieplne i pomieszczenia</b><span>Oddziel powierzchnie ogrzewane od nieogrzewanych. Suma ogrzewanych stref tworzy Af.</span></div><button onClick={addZone}>+ Dodaj strefę</button></div>
   <div className="zones-list">{zones.map((zone,index)=><div className={'zone-card '+zone.type} key={zone.id}><div className="zone-title"><span>STREFA {index+1}</span><em>{zone.source}</em>{zones.length>1&&<button onClick={()=>removeZone(zone.id)}>Usuń</button>}</div><label><span>Nazwa strefy / pomieszczenia</span><input value={zone.name} onChange={e=>updateZone(zone.id,'name',e.target.value)}/></label><div className="zone-grid"><label><span>Sposób użytkowania</span><select value={zone.type} onChange={e=>updateZone(zone.id,'type',e.target.value)}><option value="heated">Ogrzewana</option><option value="unheated">Nieogrzewana</option></select></label><label><span>Powierzchnia</span><div><input value={zone.area} placeholder="m²" onChange={e=>updateZone(zone.id,'area',e.target.value)}/><em>m²</em></div></label><label><span>Temperatura</span><div><input value={zone.temperature} onChange={e=>updateZone(zone.id,'temperature',e.target.value)}/><em>°C</em></div></label></div><small>{zone.type==='heated'?'Wliczana do powierzchni ogrzewanej Af.':'Niewliczana do Af; uwzględniana jako strefa sąsiadująca.'}</small></div>)}</div>
   <div className="zones-summary"><span>Powierzchnia ogrzewana ze stref</span><b>{zones.filter(z=>z.type==='heated').reduce((sum,z)=>sum+(Number(String(z.area).replace(',','.'))||0),0).toFixed(2)} m²</b></div>
   <div className="energy-fields">{fields.map(field=>{const value=values[field.id]??field.value,editable=!['ifc','result'].includes(field.source);return <label key={field.id} className={'energy-field '+field.source}><span>{field.label}<i>{labels[field.source]||'DANE'}</i></span><div><input value={value} readOnly={!editable} placeholder="Uzupełnij" onChange={e=>{onChange(field.id,e.target.value);setCalcState('idle')}}/>{field.unit&&<em>{field.unit}</em>}</div>{field.source==='ifc'&&<small>Odczyt z modelu IFC</small>}</label>})}</div>
  </div>:step==='systems'?<div className="systems-guide">
