...
function App(){
 ...
 const loadGeoportal=async()=>{setGeoLoading(true);setGeoReload(v=>v+1);try{const response=await fetch(`/api/geoportal?type=parcel&lat=${encodeURIComponent(gps.lat)}&lon=${encodeURIComponent(gps.lon)}`);const data=await response.json();if(!response.ok)throw new Error(data.error||'Błąd Geoportalu');setParcel(data)}catch(error){setParcel({error:error.message})}finally{setGeoLoading(false)}}
