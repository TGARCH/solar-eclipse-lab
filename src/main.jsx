...
function App(){
 const [selectedIfc,setSelectedIfc]=useState(null),[ifcState,setIfcState]=useState({status:'Oczekiwanie na model',error:null,meshes:0}),[tab,setTab]=useState('main')
 ...
 <section className="stage"><Canvas shadows dpr={[1,1.75]} camera={{position:[11,8,14],fov:46}} gl={{antialias:true,toneMapping:THREE.ACESFilmicToneMapping}}><color attach="background" args={['#0a1118']}/><Suspense fallback={null}><IfcViewer selectedId={selectedIfc?.id} onSelect={setSelectedIfc} onState={handleIfcState}/></Suspense></Canvas>
