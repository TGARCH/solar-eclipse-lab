...
export default function IfcViewer({ selectedId, onSelect, onState, sectionPlane, viewMode='model', siteRotation=0, gps={lat:'52.25',lon:'21'}, geoLayers={}, geoReload=0, mapSize=250, parcel=null, solar={date:'03-21',hour:12,all:false}, topViewSignal=0, siteOffset={x:0,y:0} }) {
  ...
  useEffect(()=>{
    ...
    const north=THREE.MathUtils.degToRad(siteRotation),stabilizer=Math.max(1,height*.012)
    // Keep the view visually top-down and north-up, but avoid the exact orbital pole where azimuth is undefined.
    camera.up.set(0,1,0);camera.position.set(Math.sin(north)*stabilizer,height,Math.cos(north)*stabilizer);camera.near=.05;camera.far=Math.max(2000,mapSize*8);camera.lookAt(0,0,0);camera.updateProjectionMatrix()
    if(controls){controls.target.set(0,0,0);controls.enableDamping=false;controls.update();controls.enableDamping=true}
  ...
  return <>
    ...
    {viewMode==='site'&&<group key={gpsFrameKey} name="GPS local frame — origin 0,0,0" position={[0,0,0]} rotation={[0,THREE.MathUtils.degToRad(siteRotation),0]}>
      <group name="Map offset in east-north axes" position={[siteOffset.x,0,-siteOffset.y]}>
        ...
        <mesh position={[0,.018,0]} rotation={[-Math.PI/2,0,0]} receiveShadow renderOrder={5}><planeGeometry args={[mapSize,mapSize]}/><shadowMaterial transparent opacity={solar.all?.28:.34} depthWrite={false}/></mesh>
